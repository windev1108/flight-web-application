    ...
    <Box sx={{ p: 3, width: '100%' }}>
      <Grid container spacing={2}>
        {Array.from({ length: 2 }).map((_, index) => (
          <Grid key={index} size={6} sx={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <Skeleton animation="wave" variant="rectangular" height={160} sx={{ mt: 1, borderRadius: 2 }} />
          </Grid>
        ))}
      </Grid>
      ...
      <Stack>
        {Array.from({ length: 3 }).map((_, index) => (
          <Skeleton animation="wave" key={index} variant="rectangular" height={200} sx={{ mt: 1, borderRadius: 2 }} />
        ))}
      </Stack>
    </Box>
  )
